...
// 둥근 손글씨 글꼴(주아체). 캔버스는 CSS의 font-family를 자동으로 따르지 않아서,
// 캔버스에 글씨를 쓰는 모든 곳이 이 값을 써야 화면 전체의 글꼴이 하나로 맞는다.
...
export function drawChunkyText(ctx, text, x, y, size, fill, opts = {}) {
  ...
  ctx.font = `900 ${size}px ${opts.font ?? FONT}`;
...
export function drawCuteBanner(ctx, dims, lines, fluidPx, opts = {}) {
  ...
  for (const m of measured) {
    if (m.chunky) {
      ...
      ctx.font = `${m.weight ?? "900"} ${m.size}px ${FONT}`;
